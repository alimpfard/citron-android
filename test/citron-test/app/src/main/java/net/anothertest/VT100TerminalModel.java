...
public class VT100TerminalModel extends AbstractTerminalModel {
    ...
    @NotNull
    public SpannableStringBuilder asSpannable() {
        sb.clear();
        int currentFg = getDefaultForegroundColor(), currentBg = getDefaultBackgroundColor();
        boolean bgch = false, fgch = true;
        int lastColorStart = 0;
        int currentIdx = 0;
        for (int line = 0; line < getRows(); line++) {
            for (int ch = 0; ch < getColumns(); ch++) {
                TerminalCell cell = getCell(ch, line);
                if (cell == null) {
                    sb.append(' ');
                    currentIdx++;
                    continue;
                ...
                currentIdx++;
                if (cell.getBackgroundColor() != currentBg)
                    bgch = true;
                if (cell.getForegroundColor() != currentFg)
                    fgch = true;
                if (fgch || bgch) {
                    sb.setSpan(new BackgroundColorSpan(cell.getBackgroundColor()), lastColorStart, currentIdx, Spannable.SPAN_MARK_MARK);
                    sb.setSpan(new ForegroundColorSpan(cell.getForegroundColor()), lastColorStart, currentIdx, Spannable.SPAN_MARK_MARK);
                    fgch = false;
                    bgch = false;
                    lastColorStart = currentIdx;
                }
            }
        }
        return sb;
    ...
    private class Vt100Listener implements AnsiControlSequenceListener {
        ...
        public void parsedString(String str) {
            for (char ch : str.toCharArray()) {
                ...
                int back = backgroundBold ? SgrColor.COLOR_BRIGHT[backgroundColor] : SgrColor.COLOR_NORMAL[backgroundColor];
                int fore = foregroundBold ? SgrColor.COLOR_BRIGHT[foregroundColor] : SgrColor.COLOR_NORMAL[foregroundColor];
     ...
     */
    private static final int DEFAULT_BACKGROUND_COLOR = 10;
     ...
     */
    private BellStrategy bellStrategy = new BellStrategy() {
        @Override
